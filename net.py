...
def generate_graph(topo: str, link_rate):
    if topo == "CEV":
        ...
        graph = _generate_graph(RandomGraph.RRG, link_rate, d=4, n=60)
    elif topo == "ERG":
        graph = _generate_graph(RandomGraph.ERG, link_rate, n=60, p=0.25)
    elif topo == "BAG":
        graph = _generate_graph(RandomGraph.BAG, link_rate, n=60, m=3)
    elif topo == "Random":
...
class FlowGenerator:
    ...
    def _generate_flow(self):
        ...
        if self.jitters is not None:
            ...
            theta = random.choice(theta_values)
            jitter = math.ceil(theta * period)
